...
* public interface ArticleRepository extends JpaRepository<Article, Long> { }:
* ArticleRepository 인터페이스를 정의합니다. 이 인터페이스는 JpaRepository를 확장하며,
* 제네릭 형태로 엔티티 클래스(Article)와 해당 엔티티의 기본 키 타입(Long)을 지정합니다.
* 이렇게 함으로써 Spring Data JPA가 Article 엔티티와 관련된 데이터베이스 작업을 처리할 수 있도록 합니다.

* ArticleRepository는 이제 Article 엔티티와 관련된 데이터베이스 작업을 쉽게 수행할 수 있는 메서드를 상속받고 있으며,
* 이 인터페이스를 사용하여 데이터베이스에서 게시물 정보를 조회, 저장, 수정, 삭제할 수 있습니다.
